...
export class IngredientConstructService {
	...
	public createNewIngredient(
		...
		const ingredientState: ReferenceItemFull = ingredientStateRef.find(
			(state: ReferenceItemFull) =>
				state.title.toLowerCase() === spoon.consistency.toLowerCase()
		);

		return {
			id: 0,
			name: basicInfo.name,
			foodGroup: foodGroupRef.find(
				(food: ReferenceItemFull) => food.id === basicInfo.foodGroup
			),
			...
			ingredientConversions: this.conversions(
				spoonConversions,
				ingredientState,
				measurementRef
			)
	...
	public conversions(
		...
		const findMeasureModel: (title: string) => MeasurementModel = (
			title: string
		) => {
		...
		const returnConvert: Conversion[] = convert.map(
			(item: ISpoonConversion, idx: number) => {
				if (item.sourceUnit === "") {
